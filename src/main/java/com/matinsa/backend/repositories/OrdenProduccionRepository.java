...
public interface OrdenProduccionRepository extends JpaRepository<OrdenProduccion,Long> {
    ...
    @Query(value = "SELECT o.id_orden as idOrden, c.nombre as nombreCliente, TO_CHAR(o.fecha_entrega, 'DD/MM/YYYY')  as fechaEntrega, TO_CHAR(l.fecha_ingreso, 'DD/MM/YYYY') as fechaIngreso, TO_CHAR(o.fecha_finalizacion, 'DD/MM/YYYY') as fechaFinalizacion, o.estado, p.nombre_producto as nombreProducto FROM orden_produccion o LEFT JOIN linea_produccion l ON l.id_orden_produccion = o.id_orden INNER JOIN cliente c ON c.id_cliente = o.id_cliente inner join producto p on p.id_producto = o.id_producto where (:estado IS NULL OR o.estado = :estado) AND (:fecha IS NULL OR DATE(o.fecha_entrega) = CAST(:fecha AS DATE))",nativeQuery = true)
